...
function calcOTIF(viajesRows, wd) {
  ...
  viajesRows.forEach(function(r) {
    ...
    // Contar todos los registros con cita capturada
    if (!cita || cita === "0" || cita === "—" || cita === "") return;
    total++;
    
    // Si no está finalizado = pendiente, no cuenta en OTIF aún
    if (!fin) { sinFecha++; return; }

    // Finalizado sin fecha real = a tiempo (ya entregó)
...
function getViajesSemana(viajesRows, cm, wd) {
  // Semanas válidas: semana actual y semana anterior (para transición)
  var semanasValidas = [String(wd.weekNum), String(wd.weekNum - 1)];
  
  var filtered = viajesRows.filter(function(r) {
    // Método 1: número de semana en col "Semana"
    var semR = String(r["Semana"] || "").trim().replace(/\.0$/, "").replace(/\./, "");
    if (semR && semanasValidas.indexOf(semR) !== -1) return true;
    
    // Método 2: fecha de carga en rango semana actual
    var f = String(r["Fecha de carga"] || "").slice(0, 10);
    if (f && wd.dates.indexOf(f) !== -1) return true;
    
    // Método 3: fecha principal del viaje
    var fV = String(r["Fecha"] || "").slice(0, 10);
    if (fV && wd.dates.indexOf(fV) !== -1) return true;
    
    // Método 4: fecha de descarga en rango (viajes que terminaron esta semana)
    var fD = String(r["Fecha descarga"] || "").slice(0, 10);
    if (fD && wd.dates.indexOf(fD) !== -1) return true;
    
    return false;
  });
  
  // Si no hay viajes filtrados, devolver todos (evitar pantalla vacía)
...
function doGet(e) {
  try {
    ...
    if (tab === "resumen_completo") {
      ...
      // Leer venta semana anterior desde celda fija O2 de Estatus_diario
      var ventaHist  = leerVentaHistorica();
      if (ventaHist.ventaSemAnt > 0) {
        // Reemplazar el semAnt calculado con el valor manual (más preciso)
        venta.semAnt.TOTAL   = ventaHist.ventaSemAnt;
        venta.semAnt.historico = ventaHist.historico;
        // Recalcular pctVsAnt
        venta.pctVsAnt = venta.semAnt.TOTAL > 0
          ? ((venta.semana.TOTAL - venta.semAnt.TOTAL) / venta.semAnt.TOTAL * 100).toFixed(1)
          : "—";
      }
