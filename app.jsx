...
const initData = () => { const s=ld(); if(s&&s.v===10)return s; return {v:10,resumen:null,cajasList:[],viajesList:[],alertasList:[],mttoList:[],lastSync:""}; };
...
const apiAppend = async (tab,row) => { await fetch(SHEETS_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({tab,action:"append",rows:[row]})}); };
...
const SyncBanner=({state,onSync,lastSync,autoInterval,setAutoInterval})=>{
  // Reloj movido al HeaderReloj — SyncBanner solo muestra estado de sync
  const cfg={idle:{bg:"#0a1628",border:"#1e3a5f",col:"#3b82f6",text:USAR_SHEETS?`☁️ Sheets${lastSync?" · "+lastSync:""}`:"💾 Local"},syncing:{bg:"#0a1f0f",border:"#10b98140",col:"#10b981",text:"🔄 Sincronizando..."},ok:{bg:"#0a1f0f",border:"#10b98140",col:"#10b981",text:"✅ Actualizado"},error:{bg:"#1f0a0a",border:"#ef444440",col:"#ef4444",text:"⚠️ Error de conexión"}};
  const c=cfg[state]||cfg.idle;
  return <div style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:9,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:8,flexWrap:"wrap"}}>
    <span style={{color:c.col,fontSize:11}}>{c.text}</span>
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
      
      {USAR_SHEETS&&<>
        <select value={autoInterval} onChange={e=>setAutoInterval(Number(e.target.value))} style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:6,padding:"3px 8px",color:"#64748b",fontSize:10,outline:"none"}}>
          <option value={0}>Manual</option><option value={30}>30 seg</option><option value={60}>1 min</option><option value={120}>2 min</option><option value={300}>5 min</option>
        </select>
        <button onClick={onSync} disabled={state==="syncing"} style={{background:"#1e293b",border:"none",borderRadius:6,padding:"4px 12px",color:"#94a3b8",fontSize:11,cursor:"pointer"}}>🔄 Sincronizar</button>
      </>}
    </div>
  </div>;
};
...
const Indicadores=({data})=>{
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {/* Venta semana actual vs anterior + histórico */}
      <div style={{background:"#0a1628",border:"1px solid #10b98130",borderRadius:12,padding:16}}>
        <div style={{fontSize:11,color:"#64748b",textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>
          💵 Venta Semana {ind.weekNum} vs Semana anterior
          {ind.ventaSemAnt>0&&<span style={{color:"#334155",fontSize:9,marginLeft:8}}>(Sem ant. capturado manualmente en Sheets col O)</span>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
          ...
          <div style={{background:"#0d1626",borderRadius:9,padding:14,textAlign:"center"}}>
            <div style={{color:ind.ventaSemAnt>0?"#f1f5f9":"#334155",fontWeight:900,fontSize:26}}>
              {ind.ventaSemAnt>0?fmt$(ind.ventaSemAnt):"—"}
            </div>
            <div style={{color:"#64748b",fontSize:10,marginTop:2}}>
              Semana anterior
              {ind.ventaSemAnt===0&&<div style={{color:"#334155",fontSize:8,marginTop:2}}>Captura en Sheets col O fila 2</div>}
            </div>
          </div>
          <div style={{background:vsCol+"15",border:`1px solid ${vsCol}40`,borderRadius:9,padding:14,textAlign:"center"}}>
            {ind.ventaSemAnt>0
              ?<><div style={{color:vsCol,fontWeight:900,fontSize:26}}>{pctVs>=0?"+":""}{ind.pctVsAnt}%</div>
                <div style={{color:"#64748b",fontSize:10,marginTop:2}}>{pctVs>=0?"▲ Crecimiento":"▼ Caída"}</div>
                <div style={{color:"#334155",fontSize:9,marginTop:1}}>Δ {fmt$(ind.ventaSemana-ind.ventaSemAnt)}</div></>
              :<><div style={{color:"#334155",fontWeight:900,fontSize:20}}>—</div>
                <div style={{color:"#334155",fontSize:9,marginTop:4}}>Sin dato semana anterior</div></>
            }
          </div>
        </div>
        {/* Venta por coordinador */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          {["TELLO","CRISTIAN","JULIO"].map(k=>{
            const col=C[k];
            const s=venta?.semana?.[k]||0;
            const a=venta?.semAnt?.[k]||0;
        ...
        {/* Histórico semanas capturadas */}
        {(data.resumen?.ventaHistorico||[]).length>0&&(
          <div>
            <div style={{color:"#334155",fontSize:9,textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>📅 Histórico capturado</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {(data.resumen?.ventaHistorico||[]).map((h,i)=>(
                <div key={i} style={{background:"#0d1626",borderRadius:7,padding:"6px 12px",textAlign:"center"}}>
                  <div style={{color:"#f1f5f9",fontWeight:700,fontSize:13}}>{fmt$(h.total)}</div>
                  <div style={{color:"#475569",fontSize:8}}>{h.semana}</div>
                </div>
              ))}
            </div>
          </div>
...
const Dashboard=({data,setTab})=>{
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {/* Entregas vencidas — igual que v8: listado + botón ver todas */}
        ...
        <div style={{background:"#1f0a0a",border:"1px solid #ef444450",borderLeft:"4px solid #ef4444",borderRadius:9,padding:"12px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <div style={{color:"#ef4444",fontWeight:700,fontSize:13}}>
              🔴 {entregas.totalVencidas} entregas VENCIDAS de {entregas.totalViajes} — {entregas.pctCumplimiento}% cumplimiento OTIF
            </div>
            <button onClick={()=>setModalVenc(true)}
              style={{background:"#ef444420",border:"1px solid #ef444460",borderRadius:6,padding:"4px 10px",color:"#ef4444",fontSize:10,cursor:"pointer",fontWeight:700,whiteSpace:"nowrap"}}>
              Ver todas ↗
            </button>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {(entregas.vencidas||[]).slice(0,4).map((v,i)=>(
              <div key={i} style={{background:"#2d0a0a",borderRadius:7,padding:"6px 10px",fontSize:10}}>
                ...
                {v.cita&&<span style={{color:"#64748b",marginLeft:6,fontSize:8}}>Cita: {v.cita}</span>}
              </div>
            ))}
            {entregas.totalVencidas>4&&(
              <span style={{color:"#64748b",fontSize:10,alignSelf:"center"}}>
                +{entregas.totalVencidas-4} más
              </span>
            )}
          </div>
        </div>
      )}
      {/* Modal — detalle completo de todas las vencidas */}
      {modalVenc&&<Modal title={`🔴 ${entregas?.totalVencidas} Entregas Vencidas — ${entregas?.pctCumplimiento}% OTIF`} onClose={()=>setModalVenc(false)} wide>
        <div style={{marginBottom:10,display:"flex",gap:12,flexWrap:"wrap"}}>
          <div style={{background:"#ef444420",borderRadius:8,padding:"8px 14px",textAlign:"center"}}>
            <div style={{color:"#ef4444",fontWeight:900,fontSize:22}}>{entregas?.totalVencidas}</div>
            <div style={{color:"#64748b",fontSize:9}}>Vencidas</div>
          </div>
          <div style={{background:"#10b98120",borderRadius:8,padding:"8px 14px",textAlign:"center"}}>
            <div style={{color:"#10b981",fontWeight:900,fontSize:22}}>{entregas?.aTiempo?.length||0}</div>
            <div style={{color:"#64748b",fontSize:9}}>A tiempo</div>
          </div>
          <div style={{background:"#3b82f620",borderRadius:8,padding:"8px 14px",textAlign:"center"}}>
            <div style={{color:"#3b82f6",fontWeight:900,fontSize:22}}>{entregas?.totalViajes||0}</div>
            <div style={{color:"#64748b",fontSize:9}}>Total con cita</div>
          </div>
          <div style={{background:"#6366f120",borderRadius:8,padding:"8px 14px",textAlign:"center"}}>
            <div style={{color:"#6366f1",fontWeight:900,fontSize:22}}>{entregas?.pctCumplimiento||0}%</div>
            <div style={{color:"#64748b",fontSize:9}}>OTIF</div>
          </div>
        </div>
        <TablaDetalle rows={entregas?.vencidas||[]} cols={[
          ...
          {k:"circuito",l:"Circuito",col:r=>r.circuito&&r.circuito!=="Sin circuito"?"#a78bfa":"#334155",bold:true},
...
// ── ALERTAS v2 — Seguimiento / Finalizado / Comentarios desde hoja ALERTAS ──
...
const HeaderReloj=({lastSync,autoInterval})=>{
  ...
  const fecha=`${dia}, ${ahora.getDate()} de ${MESES_ES[ahora.getMonth()]} de ${ahora.getFullYear()}`;
  return(
    <div style={{background:"#08111f",borderBottom:"1px solid #0f1e33",padding:"10px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:8}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:2}}>
        <div style={{fontSize:14,fontWeight:900,color:"#f1f5f9",letterSpacing:-.5}}>🚚 Nacional Autotransporte</div>
        <div style={{fontSize:9,color:"#334155",letterSpacing:1.5,textTransform:"uppercase"}}>
          ERP TMS v10 {USAR_SHEETS?`· ☁️ ${lastSync}`:"· 💾 Local"}{autoInterval>0?` · 🔄 Auto ${autoInterval}s`:""}
        </div>
      </div>
      {/* Centro: hora grande + fecha */}
      <div style={{textAlign:"center",flex:1}}>
        <div style={{fontSize:28,fontWeight:900,color:"#f1f5f9",fontFamily:"monospace",letterSpacing:2,lineHeight:1,textShadow:"0 0 20px #3b82f640"}}>
          {hora}
        </div>
        <div style={{fontSize:10,color:"#475569",marginTop:3,letterSpacing:.5,textTransform:"uppercase"}}>
          {fecha}
        </div>
      </div>
      {/* Derecha: indicador operativo */}
...
const APP_TABS=[
  ...
  {id:"ventanas",     label:"Ventanas Op.", icon:"🏗️"},
  ...
  return(
    <div style={{minHeight:"100vh",background:"#060d1a",color:"#e2e8f0",fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      ...
      <div style={{padding:16,maxWidth:1400,margin:"0 auto"}}>
        ...
        {tab==="ventanas"      &&<VentanasOperativas data={data} setTab={setTab}/>}
      ...
      <div style={{padding:"12px 18px",borderTop:"1px solid #0f1e33",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{color:"#1e3a5f",fontSize:10}}>{data.resumen?.flota?.total||0} Unidades · {data.resumen?.flota?.enOperacion||0} Operando · {(data.cajasList||[]).length} Cajas · Sem {data.resumen?.weekNum||"—"} · v10</span>
