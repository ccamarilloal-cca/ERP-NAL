...
function calcKML(rendRows, activeUnits) {
  ...
  var res={};
  Object.keys(por).forEach(function(u){var v=por[u].vals; res[u]=(v.reduce(function(a,b){return a+b;},0)/v.length).toFixed(2);});;
  return res;
}
...
function calcCoordinadores(flota, cajasRows, viajesRows, clientesRows, circuitosRows, venta, cm, wd) {
  ...
  Object.keys(coords).forEach(function(ck){
    ...
    // Circuitos dinámicos
    var cirsC=circuitosRows.filter(function(r){return String(r["Coordinador"]||"").toUpperCase().includes(cjK);});
    c.circuitos=[...new Set(cirsC.map(function(r){return r["Circuito"]||"";}).filter(Boolean))];
    var cirsActivas=activasU.map(function(u){return u.circuito;}).filter(function(ci){return ci&&ci!=="Sin circuito";});
    c.circuitos=[...new Set(c.circuitos.concat(cirsActivas))].slice(0,10);
