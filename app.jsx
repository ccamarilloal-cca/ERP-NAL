...
// ── MANTENIMIENTO v2 — fechas compromiso + semáforo + gestión ─────────────────
const Mantenimiento=({data,setData})=>{
  ...
  const enrichRow=(row)=>{
    const fc=row.fechaCompromisoUnidad||row["Fecha compromiso unidad"]||"";
    const fco=row.fechaCompromisoOperador||row["Fecha compromiso operador"]||"";
    const dias=diasDesde(fc);
    const sem=semaforoDias(dias);
    const est=mttoEstados[row.unidad]||{};
    return {...row,fechaCompromiso:fc,fechaCompromisoOp:fco,diasAtraso:dias,semaforo:sem,
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {modalGest&&(
        <Modal title={`🔧 Gestión MTTO — ${modalGest.unidad}`} onClose={()=>setModalGest(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><div style={{color:"#475569",fontSize:9,textTransform:"uppercase",marginBottom:3}}>Tipo</div><Badge text={modalGest.motivo||selTab}/></div>
              <div><div style={{color:"#475569",fontSize:9,textTransform:"uppercase",marginBottom:3}}>Fecha compromiso</div><div style={{color:modalGest.semaforo?.col||"#f1f5f9",fontWeight:700}}>{modalGest.fechaCompromiso||"—"}</div></div>
              <div><div style={{color:"#475569",fontSize:9,textTransform:"uppercase",marginBottom:3}}>Estado</div><div style={{color:modalGest.semaforo?.col||"#475569",fontWeight:700}}>{modalGest.semaforo?.label||"—"}</div></div>
              <div><div style={{color:"#475569",fontSize:9,textTransform:"uppercase",marginBottom:3}}>Comentario Sheets</div><div style={{color:"#94a3b8",fontSize:11}}>{modalGest.comentarios||"—"}</div></div>
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              <label style={{fontSize:11,color:"#64748b",textTransform:"uppercase"}}>Comentario seguimiento</label>
            ...
            <button onClick={guardarGestion} style={{background:"#3b82f620",border:"1px solid #3b82f660",borderRadius:8,padding:"10px",color:"#3b82f6",fontWeight:700,cursor:"pointer",fontSize:12}}>
              💾 Guardar {USAR_SHEETS?"+ actualizar Sheets":""}
            </button>
      ...
      <div style={{background:"#0a1628",border:"1px solid #1e3a5f",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#3b82f6"}}>
        ℹ️ MTTO = CP + RM + SG · Fechas compromiso desde hoja MANTENIMIENTO de Sheets
      </div>
      ...
      {atrasadas.length>0&&(
        <div style={{background:"#1a1100",border:"1px solid #f59e0b40",borderRadius:10,padding:"12px 14px"}}>
          <div style={{color:"#f59e0b",fontWeight:700,fontSize:12,marginBottom:8}}>📅 {atrasadas.length} con fecha compromiso VENCIDA</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {atrasadas.map((r,i)=>(
              <div key={i} style={{background:"#2d1a00",borderRadius:7,padding:"6px 10px",fontSize:10}}>
                ...
                <span style={{color:"#64748b",marginLeft:6}}>{r.fechaCompromiso}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          :<div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead><tr style={{borderBottom:"2px solid #1e293b"}}>
                {["Unidad","Operador","Coord","Motivo","F.Compromiso","Días","Estado","Nuevo F.","Circuito","Gestión"].map(h=><th key={h} style={{textAlign:"left",padding:"7px 8px",color:"#475569",fontSize:9,textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>)}
              </tr></thead>
              <tbody>{filasEnriquecidas.map((e,i)=>(
                <tr key={e.unidad+i} style={{borderBottom:"1px solid #0d1626",background:i%2===0?"#080e1c":"transparent"}}>
                  ...
                  <td style={{padding:"8px 8px",color:"#94a3b8",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.operador}</td>
                  ...
                  <td style={{padding:"8px 8px",color:"#64748b",fontSize:9}}>{e.nuevaFecha||"—"}</td>
