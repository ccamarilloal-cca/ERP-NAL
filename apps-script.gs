...
function calcOTIF(viajesRows, wd) {
  ...
  viajesRows.forEach(function(r) {
    ...
    if (!fin) return;
    total++;
    if (!cita || cita === "0") { sinFecha++; return; }
    if (!real) { onTime++; return; }
  ...
  var pct = (onTime + late) > 0 ? Math.round(onTime / (onTime + late) * 100) : 100;
...
function calcRanking(dieselRows, viajesRows, rendRows, catalogMap) {
  ...
  Object.keys(catalogMap).forEach(function(uN) {
    opMap[uN] = {
      ...
      kmlVals: [], rendViajeVals: [], viajes: 0,
      kmTotal: 0, totalLitros: 0, ventaTotal: 0,
      ultimaFechaCarga: "", ultimoRendimiento: "", ultimoViaje: ""
    };
  });
  ...
  dieselRows.forEach(function(r) {
    ...
    if (kml > 0 && kml <= 8) { // filter unrealistic
      opMap[uN].kmlVals.push(kml);
      if (fecha > opMap[uN].ultimaFechaCarga) {
  ...
  rendRows.forEach(function(r) {
    ...
    if (kml > 0 && kml <= 8) opMap[uN].kmlVals.push(kml);
    opMap[uN].kmTotal += km;
  });
  ...
  viajesRows.forEach(function(r) {
    ...
    var vta = parseMonto(r["Venta real"] || r["Venta estimada"]);
    opMap[uN].kmTotal += km;
    opMap[uN].ventaTotal += vta;
    var fecha = String(r["Fecha de carga"] || "").slice(0, 10);
    if (fecha > opMap[uN].ultimoViaje) opMap[uN].ultimoViaje = fecha;
    if (km > 0 && opMap[uN].totalLitros > 0) {
      opMap[uN].rendViajeVals.push(km / (opMap[uN].totalLitros / Math.max(opMap[uN].viajes,1)));
    }
  });
  ...
  return Object.values(opMap).map(function(d) {
    ...
    var rendVj = d.rendViajeVals.length > 0
      ? (d.rendViajeVals.reduce(function(a,b){return a+b;},0) / d.rendViajeVals.length).toFixed(1) : "—";
    return {
      unidad: d.unidad, operador: d.operador, coordinador: d.coordinador,
      rendimientoKmLt: kmlProm, rendimientoViaje: rendVj,
      viajesCompletados: d.viajes, kmTotal: Math.round(d.kmTotal),
      totalLitros: Math.round(d.totalLitros), ventaTotal: Math.round(d.ventaTotal),
      ultimaFechaCarga: d.ultimaFechaCarga, ultimoRendimiento: d.ultimoRendimiento,
